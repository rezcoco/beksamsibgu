        ...
        return [
          {
            sourceId: "vocabulary",
            getItemUrl: ({ item }) => {
              return `/kosa-kata/${item.objectID}`;
            },
            onSelect: navigate,
            getItems({ query }) {
...
function HighlightQuery({ text, query }: { text: string; query: string }) {
  return (
    <Highlighter
      highlightClassName="bg-emerald-200 dark:bg-emerald-500 dark:text-white"
      searchWords={[query]}
      autoEscape={true}
    ...
    >
      <div className="flex flex-col">
        <div
          id={`${id}-title`}
          aria-hidden="true"
          className="text-sm text-zinc-700 dark:text-white font-bold"
        >
          <HighlightQuery text={result.hangeul} query={query} />
        </div>
        <div className="text-xs text-zinc-700 dark:text-white">
          <HighlightQuery text={result.translation} query={query} />
        </div>
      </div>
    </li>
  );
  ...
  if (collection.items.length === 0) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="mt-2 text-xs dark:text-zinc-400">
          Nothing found for{" "}
...
>(function SearchInput({ autocomplete, autocompleteState, onClose }, inputRef) {
  ...
  return (
    <div className="group relative flex h-12">
      <SearchIcon className="pointer-events-none absolute left-3 top-0 h-full w-5 stroke-zinc-700 dark:stroke-zinc-400" />
      <input
        ref={inputRef}
        data-autofocus
        className={clsx(
...
function SearchDialog({
  ...
  return (
    ...
    >
      <DialogBackdrop
        transition
        className="fixed inset-0 bg-zinc-700 dark:text-zinc-400/25 backdrop-blur-sm data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in dark:bg-black/40"
      />
...
export function Search() {
  ...
  return (
    <div className="hidden lg:block lg:max-w-md lg:flex-auto">
      <button
        type="button"
        className="hidden h-8 w-full items-center gap-2 rounded-full bg-white pl-2 pr-3 text-sm text-zinc-700 dark:text-zinc-400 ring-1 ring-zinc-900/10 transition hover:ring-zinc-900/20 ui-not-focus-visible:outline-none lg:flex dark:bg-white/5 dark:ring-inset dark:ring-white/10 dark:hover:ring-white/20"
        {...buttonProps}
      >
        <SearchIcon className="h-5 w-5 stroke-current" />
        Cari kosa kata...
        <kbd className="ml-auto text-2xs text-zinc-700 dark:text-zinc-400">
